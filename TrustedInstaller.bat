...
if ($isTurkish) {
    $msg = @{
        ...
        PopupMsg = @"
Bu script'i kullanmak için herhangi bir dosyayı üzerine sürükleyip bırakın.
...
"@
    ...
    $msg = @{
        ...
        PopupTitle = "Trusted Installer Launcher | made by Abdullah ERTÜRK | TNCTR.com"
        PopupMsg = @"
Drag and drop any file onto this script to run it.
...
function RunAsTI ($cmd) {
    ...
    # Bekleme popup'ını başlat (ayrı process) - 5 saniye timeout ile
    $waitPopupScript = @"
...
"@
    ...
    # 5 saniye sonra MessageBox'u kapat
    $timeoutScript = @"
    ...
    if ($ti) {
        ...
        if ($isCMD) {
            ...
            if (!$cmdArgs -or $cmdArgs -eq '' -or $cmdArgs -eq '/k') {
                ...
                $argList = "/k title `"$titleForCmd`""
            } else {
                # Mevcut argümanlara title ekle
                $titleForCmd = $msgTitle -replace '"','""'
                $argList = "$cmdArgs && title `"$titleForCmd`""
            }
...
'@
    ...
    try {
        ...
        Set-ItemProperty $key $id $($a1, $a2, $a3, $payload) -Type 7 -Force
        "Registry set successfully" | Out-File $logFile -Append
        
        $arg = "$a1 `$env:A = (Get-ItemProperty '$key').'$id' -join ''; Remove-ItemProperty '$key' '$id' -Force; iex `$env:A"
...
"@
    ...
    if ($result -eq 6) {
        ...
        if ($choice -eq 6) { 
            ...
            RunAsTI "explorer.exe `"$explorerPath`""
        }
